...
bool BaseEntity::Shoot() {
    ...
    switch (shootBulletType_) {
        ...
        case BulletType::kHoming:
            bullet = std::make_unique<HomingBullet>(bulletModel_, GetCollisionAttribute(), 0.5f,
                statusParametersCurrent_.attackPower, statusParametersCurrent_.bulletShootSpeed, statusParametersCurrent_.bulletShootInterval);
            bullet->SetTarget(*targetList_.begin());
            break;
    ...
    // 発射間隔が1/10を下回らないように制限
    shootIntervalTimer_ = std::max(shootIntervalTimer_, 1.0f / 10.0f);
    return true;
}
